##### 背景：
##### 想要研究客户的特征，从而来提升店铺销售额，但是亚马逊并没有提供客户的个人信息（个人隐私），也没有用户的行为数据，所以客户从什么渠道来的、页面逗留时间、加购情况等等，
一概不知。但是，基本的交易数据还是能拿到的。客户的邮箱（作为ID鉴别）、交易时间、交易金额都有，可以做个RFM客户价值分析，看看店铺里的客户情况是怎么样的。再根据该情况，给运
营、推广提供建议。

###### RFM可以不用python来写，excel都可以完成。但是写个脚本，以后就可以一键操作啦
该模型采用经典的二分法，有人用中位数、平均数、二八划分值，具体怎么分，看各自的店铺情况。
